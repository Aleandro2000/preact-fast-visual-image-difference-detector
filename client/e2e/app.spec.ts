...
test.describe("Home Page", () => {
  test("renders heading and get started link", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByText("Visual Diff", { exact: true })).toBeVisible();
    await expect(page.getByText("Detector", { exact: true })).toBeVisible();
    const link = page.getByRole("link", { name: /launch detector/i });
...
test.describe("Differentiator Page", () => {
  ...
  test("zoom controls work after comparison", async ({ page }) => {
    ...
    const zoomInBtn = page.locator("button").filter({ has: page.locator(".lucide-zoom-in") });
    const zoomOutBtn = page.locator("button").filter({ has: page.locator(".lucide-zoom-out") });
